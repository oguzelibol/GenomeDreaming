...
def main():
    ...
    parser.add_argument('--data_dir', type=str, default='prokaryotes/', help='data directory containing input.txt')
    ...
    parser.add_argument('--init_from', type=str, default=None, help="""continue training from saved model at this path. Path must contain files saved by previous training process: 
...
<<<<<<< HEAD
>>>>>>> 701f3388b61f964ee9164580a29e065f928f98da
=======
>>>>>>> origin/master
            ...
            #Validation loss
            total_val_loss = []
            data_loader.reset_valbatch_pointer()
            for bv in range(data_loader.num_valbatches):
                xVal, yVal = data_loader.next_valbatch()
                feed = {model.input_data: xVal, model.targets: yVal}
                for i, (c, h) in enumerate(model.initial_state):
                    feed[c] = state[i].c
                    feed[h] = state[i].h
                validation_loss = sess.run([model.cost],feed)
                total_val_loss.append(validation_loss[0])
            total_val_loss = np.mean(total_val_loss)
            valLossList.append(total_val_loss)
            print('\n******Validation Loss********')
            print(validation_loss[0])

            with open(os.path.join(args.save_dir, 'lossHistory.pkl'), 'wb') as f:
                cPickle.dump([trainLossList, valLossList], f)
